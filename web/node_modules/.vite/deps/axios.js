...
function toURLEncodedForm(data, options) {
	return toFormData$1(data, new browser_default.classes.URLSearchParams(), Object.assign({ visitor: function(value, key, path, helpers) {
...
var DEFAULT_CONTENT_TYPE = { "Content-Type": void 0 };
/**
...
var defaults = {
	...
	transformRequest: [function transformRequest(data, headers) {
		...
		if (utils_default.isArrayBuffer(data) || utils_default.isBuffer(data) || utils_default.isStream(data) || utils_default.isFile(data) || utils_default.isBlob(data)) return data;
...
utils_default.freezeMethods(AxiosHeaders$1.prototype);
...
var cookies_default = browser_default.isStandardBrowserEnv ? (function standardBrowserEnv() {
	return {
		write: function write(name, value, expires, path, domain, secure) {
			...
			if (utils_default.isNumber(expires)) cookie.push("expires=" + new Date(expires).toGMTString());
...
var isURLSameOrigin_default = browser_default.isStandardBrowserEnv ? (function standardBrowserEnv() {
...
var knownAdapters = {
	http: null,
	xhr: typeof XMLHttpRequest !== "undefined" && function(config) {
		return new Promise(function dispatchXhrRequest(resolve, reject) {
			...
			if (utils_default.isFormData(requestData) && (browser_default.isStandardBrowserEnv || browser_default.isStandardBrowserWebWorkerEnv)) requestHeaders.setContentType(false);
			...
			if (!utils_default.isUndefined(config.withCredentials)) request.withCredentials = !!config.withCredentials;
...
var adapters_default = {
	getAdapter: (adapters) => {
		...
		for (let i = 0; i < length; i++) {
			...
			if (adapter = utils_default.isString(nameOrAdapter) ? knownAdapters[nameOrAdapter.toLowerCase()] : nameOrAdapter) break;
		}
		if (!adapter) {
			if (adapter === false) throw new AxiosError$1(`Adapter ${nameOrAdapter} is not supported by the environment`, "ERR_NOT_SUPPORT");
		...
		if (!utils_default.isFunction(adapter)) throw new TypeError("adapter is not a function");
		return adapter;
	},
...
function mergeConfig$1(config1, config2) {
	...
	function mergeDeepProperties(a, b, caseless) {
		if (!utils_default.isUndefined(b)) return getMergedValue(a, b, caseless);
		else if (!utils_default.isUndefined(a)) return getMergedValue(void 0, a, caseless);
	}
...
var { Axios, AxiosError, CanceledError, isCancel, CancelToken, VERSION, all, Cancel, isAxiosError, spread, toFormData, AxiosHeaders, HttpStatusCode, formToJSON, mergeConfig } = axios;
//#endregion
export { Axios, AxiosError, AxiosHeaders, Cancel, CancelToken, CanceledError, HttpStatusCode, VERSION, all, axios as default, formToJSON, isAxiosError, isCancel, mergeConfig, spread, toFormData };
